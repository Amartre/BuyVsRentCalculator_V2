...
function drawChart() {
    ...
    for (var i = 0; i < YearsArr.length; i++) {
      data.addRow([YearsArr[i].toString(), AnnualRentIncreaseArray[i], PropertyTaxYrArray[i]]);
    }

    var options = {
        title: 'Monthly Rent vs Buy Costs',
        width: '100%', // Adjust the chart's width to fill the parent div
        height: '100%', // Adjust the chart's height to fill the parent div
        chartArea: { 
            width: '50%', // Adjust the chart area's width to fill 80% of the chart's width
            height: '50%' // Adjust the chart area's height to fill 80% of the chart's height
        },
        hAxis: {
            title: 'Year',
            minValue: 1
        ...
        vAxes: {
            0: { title: 'Cost ($)', minValue: 0 },
            1: { title: 'Cost ($)', minValue: 0 }
        },
        series: {
            0: { targetAxisIndex: 0 },
            1: { targetAxisIndex: 0 }
        },
        colors: ['#FF6D2C', '#004b8d']
    };
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function GetDownPercent() {
        //Gets downdollar and converts to percent
        dollarDown = (document.getElementById("dollardown").value).replace(/\,/g, '');
        dollarDownNum = Number(dollarDown);
        homePrice = (document.getElementById("homeprice").value).replace(/\,/g, '');
        homePriceNum = Number(homePrice);
